...
namespace TameMyCerts.Models
{
    internal class YubikeyObject
    {
        ...
        public YubikeyObject(string publicKey, byte[] AttestionCertificateByte, byte[] IntermediateCertificateByte)
        {
            // This is simple in newer .NET versions, where we can load a custom CA Root. .NET Framework does not support this.
            // Load the Yubikey certificates
            ...
            if (VerifyChain(IntermediateCertificate, YubikeyValidationCA) == false)
            {
                throw new Exception("Yubikey chain verification failed");
            }
            Console.WriteLine("blää");

            //Section to update the TouchPolicy, PinPolicy, FormFactor, Slot
            ...
            {
                this.Version = new Version(FirmwareVersion[0], FirmwareVersion[1], FirmwareVersion[2]);
            }



            Console.WriteLine($"{FirmwareVersion[0]}");
            Console.WriteLine($"{FirmwareVersion[1]}");
            Console.WriteLine($"{new Version(FirmwareVersion[0], FirmwareVersion[1], FirmwareVersion[2])}");
            Console.WriteLine($"{this.Version}");
            //AttestationCertificate.Extensions

            //  TryGetValue(YubikeyOID.ATTESTION_DEVICE, out var AttestionCertificate);
            //
